...
try:
    ...
    df_trie_price_by_date = df.groupby('Date')["CA"].sum().reset_index()
    ...
    # les prints
    print(df.head(5))  # Consigne:Afficher Les premières lignes
    print("il y a", len(df), "produits en tout") # Consigne:Afficher Le nombre total de lignes
    ...
    plt.bar(df_trie_price_by_date["Date"], df_trie_price_by_date["CA"], color='blue', label="Chiffre d'Affaires")
    plt.yticks(range(0, int(df_sorted_by_date["CA"].max()) + 1))
    plt.legend()
    plt.show()
    plt.tight_layout()
